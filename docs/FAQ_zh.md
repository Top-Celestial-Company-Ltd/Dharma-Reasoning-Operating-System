...
*   **VajraClaw 物理防禦**：
    1.  **DFA 狀態機過濾**：Hermes Agent 的輸出與輸入 Token 流被接入 `matchTokenStreamWithAudit()`。VajraClaw 使用 C/Go 實現的高效 DFA 狀態機，能在微秒級內識別並抹除 Token 流中隱藏的危險外連 URL，讓受污染的 AI 無法吐出外連路徑。
    2.  **Allowed Scopes 鎖死**：即使 AI 被完全欺騙並調用了 `execute_command` 工具。VajraClaw 檢測到當前執行的 BEC 臨時憑證中，`allowed_scopes` 不包含此目錄的寫入權限，或者命令匹配了黑名單模式，VajraClaw 會直接將該調用攔截並報警，AI 產生的危險指令根本無法抵達作業系統終端。
