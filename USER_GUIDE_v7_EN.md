...
7. **(Advanced) Inject Mahabodhi Pitaka & Vajra Precepts**: 
   If you have downloaded the full 1.6GB Mahabodhi Pitaka, or need to update the strictest Vajra Contracts, **double-click `run_vajra_injector.bat` (or `雙擊執行-DROS金剛注射器.bat`)** AFTER the Proxy is running successfully.
...
> 
> **2. `run_vajra_injector.bat` / Vajra Injector (Run Second: Mount the Pitaka)**
> *   **File Location**: In the absolute root directory of the project (Usually named `run_vajra_injector.bat` or `雙擊執行-DROS金剛注射器.bat`).
